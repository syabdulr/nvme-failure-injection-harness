...
# Fault-injection knobs (set by env/run.sh per scenario):
#   NVME_BLKDEBUG_CONF  path to a blkdebug config -> backend returns errors
#   NVME_EXTRA_PROPS    extra ",key=val" appended to the -device nvme string
NVME_BLKDEBUG_CONF="${NVME_BLKDEBUG_CONF:-}"
NVME_EXTRA_PROPS="${NVME_EXTRA_PROPS:-}"
...
build_nvme_qemu_args() {
  ...
  local drive_file="$NVME_BACKING"
  if [[ -n "$NVME_BLKDEBUG_CONF" ]]; then
    [[ -f "$NVME_BLKDEBUG_CONF" ]] || die "blkdebug conf not found: $NVME_BLKDEBUG_CONF"
    drive_file="blkdebug:$NVME_BLKDEBUG_CONF:$NVME_BACKING"
    log "backend: blkdebug ($NVME_BLKDEBUG_CONF)"
  fi
  [[ -n "$NVME_EXTRA_PROPS" ]] && log "extra nvme props: $NVME_EXTRA_PROPS"

  NVME_QEMU_ARGS=(
    -drive "file=$drive_file,if=none,id=nvm0,format=raw,discard=on,detect-zeroes=unmap"
    -device "nvme,drive=nvm0,serial=$NVME_SERIAL,id=nvme0,logical_block_size=$NVME_LBADS,physical_block_size=$NVME_LBADS,discard_granularity=$NVME_DISCARD_GRAN$ocp$NVME_EXTRA_PROPS"
  )
}
